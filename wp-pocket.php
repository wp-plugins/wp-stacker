				...
				<?php $this->support_div(); ?>
					...
					<table class="form-table">
						<tr>
							<th>Pocket Auth</th>
							<td>
								<?php if (get_option('wppocket_pocket_username') != '') { echo $this->xcrypt(1, get_option('wppocket_pocket_username')) . ' authenticated.'; } ?> 
								<?php if (get_option('wppocket_pocket_username') == '') { echo '<a href="?page=settings_pocket_poster&auth-get"><input type="button" class="button-primary" value="Authenticate account" /></a>'; }else  { echo '<a href="?page=settings_pocket_poster&auth-get"><input type="button" class="button-primary" value="Authenticate other account" /></a>'; } ?>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" size="57" name="wppocket_pocket_consumer_key" value="<?php if(get_option('wppocket_pocket_consumer_key_custom') == 1) { echo $this->xcrypt(1, get_option('wppocket_pocket_consumer_key')); } ?>" />
								
								<?php if(get_option('wppocket_pocket_consumer_key_custom') == 1) { ?>
									<label><input name="wppocket_pocket_consumer_key_custom_remove" type="checkbox" value="1" /> Use the default Consumer-Key</label>
								<?php } ?>
					...
					<h3>WP Pocket Settings</h3>
					<table class="form-table">
						<tr>
							<th>Language</th>
							<td>
								<select name="wppocket_language">
									<option value="en" <?php selected('en', get_option('wppocket_language')); ?>>English</option>
									<option value="de" <?php selected('de', get_option('wppocket_language')); ?>>German</option>
									<option value="fr" <?php selected('fr', get_option('wppocket_language')); ?>>French</option>
								</select>
								<p class="description">A few variables you can set are language-dependent. Which language should WP Pocket use? <strong>This setting does not change the language of complete WP Pocket (currently only English supported)!</strong></p>
							</td>
						</tr>
						<tr>
							<th>Statistics</th>
							<td>
								<select name="wppocket_statistics">
									<option value="1" <?php selected('1', get_option('wppocket_statistics')); ?>>Yes</option>
									<option value="0" <?php selected('0', get_option('wppocket_statistics')); ?>>No</option>
								</select>
								<p class="description">Do you allow WP Pocket to collect anonymous statistics about published links and updates and send them to WP-Pocket.com? These statistics will only be used to improve WP Pocket.</p>
							</td>
						</tr>
					</table>
					...
					<table class="form-table">
						<tr>
							...
							<td>
								<select name="wppocket_links_to_use">
									...
									<option value="1" <?php selected('1', get_option('wppocket_links_to_use')); ?>>Only read/archived links, which are unposted</option>
									<option value="2" <?php selected('2', get_option('wppocket_links_to_use')); ?> disabled><s>All links, which are unposted</s> (Currently impossible due to a bug in Pocket API)</option>
									<option value="3" <?php selected('3', get_option('wppocket_links_to_use')); ?>>All unread links, also already posted ones</option>
									<option value="4" <?php selected('4', get_option('wppocket_links_to_use')); ?>>All read/archived links, also already posted ones</option>
									<option value="5" <?php selected('5', get_option('wppocket_links_to_use')); ?> disabled><s>All links, also already posted ones</s> (Currently impossible due to a bug in Pocket API)</option>
								</select>
						...
						<tr>
							<th>Favorites</th>
							<td>
								<select name="wppocket_faved">
									<option value="0" <?php selected('0', get_option('wppocket_faved')); ?>>All links</option>
									<option value="1" <?php selected('1', get_option('wppocket_faved')); ?>>Only faved links</option>
									<option value="2" <?php selected('2', get_option('wppocket_faved')); ?>>Only unfaved links</option>
								</select>
						...
						<tr>
							...
							<td>
								...
								<p class="description">
									...
									<i>%%url%%</i> for the link itself; <i>%%tags%%</i> for added tags in pocket/metatags; <i>%%author%%</i> for the name 
						...
						<tr>
							...
							<td>
								...
								<p class="description">
									...
									<strong>Use <i>%%day%%</i> for day, on which the post is being generated; <i>%%month%%</i> for the month; <i>%%year%%</i> for the year; %%count%% to get the number of posted links.</strong>
								</p>
							</td>
						</tr>
						<tr>
			...
			<div class="wrap wppocket">
				...
				if ($_POST['mode'] == 'get') { // Get Elements
					...
					if ($checkPocketAccess) {				
						...
						$getLinksPost['consumer_key'] = $this->xcrypt(1, get_option('wppocket_pocket_consumer_key'));
						$getLinksPost['access_token'] = $this->xcrypt(1, get_option('wppocket_pocket_access_key'));
						...
						}elseif (get_option('wppocket_links_to_use') == 1 || get_option('wppocket_links_to_use') == 4) { // Readed / Archived
							$getLinksPost['state'] = 'archive';
						} // All
						
						// Check if only faved or unfaved links should use
						if (get_option('wppocket_faved') == 1) { // only faved
							$getLinksPost['favorite'] = '1';
						}else if (get_option('wppocket_faved') == 2) { // only unfaved
							$getLinksPost['favorite'] = '0';
						} // All
						
						// Check if order should be oldest to newest or newest to oldest
						if (get_option('wppocket_order')) { // oldest to newest
						...
						if (get_option('wppocket_links_to_use') == 0 || get_option('wppocket_links_to_use') == 1 || get_option('wppocket_links_to_use') == 2) {
							...
							foreach ($getLinks->list as $key => $value) {
								...
								$sql = 'SELECT `id` FROM `' . $this->wpdb->prefix . 'wppocket_posted_links` WHERE `url`="' . $value->given_url . '"';
							...
							<form id="update-nav-menu" action="" method="post" enctype="multipart/form-data" style="display: none;">
								...
								<ul class="menu ui-sortable" id="menu-to-edit">
									...
									foreach ($getLinks->list as $key => $value) {
										...
										if ($value->resolved_title == '' || $value->excerpt == '' || (get_option('wppocket_meta_keyword_as_tags') == 1 && $value->tags == '')) {
											...
											for ($i = 0; $i < $metas->length; $i++) {
												...
												if (get_option('wppocket_meta_keyword_as_tags') == 1 && $value->tags == '') {
											...
											<div class="menu-item-settings" id="item-<?php echo $key; ?>">
												...
												<p class="description description-thin description-one-thirds">
													<label for="item-<?php echo $key; ?>-input-author">
														Author<br />
														<input type="text" id="item-<?php echo $key; ?>-input-author" class="widefat input-title" name="element[][author]" value="<?php
												...
												<p class="description description-thin description-two-thirds">
													<label for="item-<?php echo $key; ?>-input-url">
														URL<br />
														<input type="text" id="item-<?php echo $key; ?>-input-url" class="widefat" name="element[][url]" value="<?php if (get_option('wppocket_clean_url') == 1) { echo $this->cleanUpURL($value->resolved_url); }else { echo $value->given_url; } ?>" />
												...
												<p class="description">
													<label for="item-<?php echo $key; ?>-input-description">
														...
														<textarea name="element[][description]" rows="4" id="item-<?php echo $key; ?>-input-description" class="widefat" type="textarea"><?php echo $value->excerpt; ?></textarea>
													</label>
												</p>
