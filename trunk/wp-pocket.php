...
if(!class_exists('wppocketer')) {
	class wppocketer {
		...
		private function xcrypt($mode, $string) {
			/*
			 * Required parameters:
			...
			// Get crypt keys
			$sql = 'SELECT `wppocket_key_a`, `wppocket_key_b` FROM `' . $this->wpdb->prefix . 'options`';
			$keys = $this->wpdb->get_row($sql);
			
			if ($mode == 0) { // Encrypt
				return base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($keys->wppocket_key_a), $string, MCRYPT_MODE_CBC, md5(md5($keys->wppocket_key_b))));
			}elseif ($mode == 1) { // Decrypt
				return rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($keys->wppocket_key_a), base64_decode($string), MCRYPT_MODE_CBC, md5(md5($keys->wppocket_key_b))), "\0");
			}else { // False mode
				return false;
			}
		}
									...
									foreach ($getLinks->list as $key => $value) {								
										...
										for ($i = 0; $i < $metas->length; $i++) {
											...
											if($meta->getAttribute('name') == 'description') {
												$value->description = utf8_decode($meta->getAttribute('content'));
											}
