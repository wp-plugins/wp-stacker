	...
	class wpstacker {
		...
		function activate() {
			...
			WHERE
				...
				option_name = "wppocket_faved" OR
				option_name = "wppocket_order" OR
				option_name = "wppocket_mark_as_readed" OR
				option_name = "wppocket_clean_url" OR
				option_name = "wppocket_meta_keyword_as_tags" OR
				option_name = "wppocket_reference_wpstacker" OR
				option_name = "wppocket_ping_status" OR
				option_name = "wppocket_comment_status" OR
				option_name = "wppocket_post_status" OR
				option_name = "wppocket_post_category" OR
				option_name = "wppocket_pocket_username" OR
				option_name = "wppocket_pocket_access_key" OR
				option_name = "wppocket_title" OR
				option_name = "wppocket_introduction_text" OR
				option_name = "wppocket_statements_text" OR
				option_name = "wppocket_entry_building" OR
				option_name = "wppocket_language" OR
				option_name = "wppocket_statistics" OR
				option_name = "wppocket_key_a" OR
				option_name = "wppocket_key_b" OR
				option_name = "wppocket_pocket_password" OR
				option_name = "wppocket_pocket_api_key" OR
				option_name = "wppocket_pocket_api_key_custom" OR
				option_name = "wppocket_pocket_consumer_key_custom" OR
				option_name = "wppocket_pocket_consumer_key"
			';
			@$this->wpdb->query($sql);
			...
			if (!version_compare(get_option('wpstacker_version'), '1.0.2', '>=')) {
				// Update Pocket username, password and api-key
				update_option('wpstacker_pocket_username', $this->xcrypt(0, $this->xcrypt(1, get_option('wpstacker_pocket_username'), true)));
				update_option('wpstacker_pocket_password', $this->xcrypt(0, $this->xcrypt(1, get_option('wpstacker_pocket_password'), true)));
				update_option('wpstacker_pocket_api_key', $this->xcrypt(0, $this->xcrypt(1, get_option('wpstacker_pocket_api_key'), true)));
			}
			...
			if (get_option('wpstacker_version') != $this->version) {
		...
		function options_link() {
			...
			add_options_page('WP Stacker Settings', 'WP Stacker', 5, 'settings_pocket_poster', array($this, 'options_page_view'));
		...
		function options_page_controller() {
			// Solve escape errors
			$_POST['wpstacker_title'] = stripslashes_deep($_POST['wpstacker_title']);
			$_POST['wpstacker_entry_building'] = stripslashes_deep($_POST['wpstacker_entry_building']);
			$_POST['wpstacker_introduction_text'] = stripslashes_deep($_POST['wpstacker_introduction_text']);
			$_POST['wpstacker_statements_text'] = stripslashes_deep($_POST['wpstacker_statements_text']);
			...
			if ($_POST['wpstacker_edit']) {
				...
				if ($_POST['wpstacker_pocket_consumer_key_custom_remove'] == 1) {
					// Set default key
		...
		function main_page_view() {
			...
			<div class="wrap wpstacker">
				...
				if ($_POST['mode'] == 'get' && $_POST['onlyWithTag']) { // Get Elements
				...
				if ($_POST['mode'] == 'get') { // Get Elements
					...
					if ($checkPocketAccess) {				
						...
						if (count((array)$getLinks->list) == 0) { // No elements
							...
							if ($_POST['onlyWithTag'] && $_POST['tag'] != '') {
					...
					if ($_POST['count_elements'] == 0) { // No elements
						...
						?>
						<p class="short"><strong>Great! You posted <?php echo $_POST['count_elements']; ?> links from your Pocket account into your blog. <?php if ($_POST['wpstacker_mark_as_readed']) { ?>[18:56:01] Elias: They are now marked as read in your pocket account!<?php } ?></strong></p>
			...
			if ($_POST['mode'] == 'save') { // Save post
				...
				while ($i < (count($_POST['element']) / 9)) {
					...
					if (get_option('wpstacker_links_to_use') == 0 || get_option('wpstacker_links_to_use') == 1 || get_option('wpstacker_links_to_use') == 2) {
						...
						$sql = 'SELECT `id` FROM `' . $this->wpdb->prefix . 'wpstacker_posted_links` WHERE `url`="' . $_POST['element'][($i * 9) + 2]['url'] . '"';
						$row = $this->wpdb->get_results($sql);
